...
def draw_My_partnet_objects(objects, object_names=None, figsize=None, rep='boxes', \
                         leafs_only=False, use_id_as_color=False, visu_edges=True, sem_colors_filename=None):
    # load sem colors if provided
    if sem_colors_filename is not None:
        sem_colors = load_semantic_colors(filename=sem_colors_filename)
    ...
    for i, obj in enumerate(objects):
        ...
        if len(objects)%2==0:
            ax = fig.add_subplot(2, len(objects) / 2, i + 1, projection='3d')
        else:
            ax = fig.add_subplot(1, len(objects), i + 1, projection='3d')
        ax.set_xlim(-extent, extent)
        ax.set_ylim(extent, -extent)
...
# path = ["C:/Users/liuzh/Desktop/aaaa.json"]
# path = ["../data/partnetdata/vase_hier/3394.json"]
## 训练集加载
# path = []
# for i in [3390,3391,3394,3397,3399, 3401, 6380]:
#     path.append("../data/partnetdata/vase_hier/{0}.json".format(i))

# 预测结果加载
path = []
for i in range(1, 5):
    path.append("../data/partnetdata/plant_hier/{0}.json".format(i))
for i in range(34, 38):
    path.append("../data/results/box_vae_plant/object-%04d.json" % i)

dic = json.load(open(path[1]))
js = json.dumps(dic, sort_keys=True, indent=4)
print(js)

tmp_objs = []
for p in path:
    tmp_objs.append(PartNetDataset.load_object(p))
draw_My_partnet_objects(tmp_objs)
# 最佳训练
# python ./train_box.py  --exp_name box_vae_plant  --category Plant  --data_path ../data/partnetdata/plant_hier   --train_dataset train.txt --epochs 20 --model_version model_box --batch_size 16 --lr 0.001


#  python ./train_box.py  --exp_name box_vae_plant  --category Plant  --data_path ../data/partnetdata/plant_hier   --train_dataset train.txt
#  --epochs 50 --model_version model_box --batch_size 4

# python ./eval_gen_box.py  --exp_name box_vae_plant   --test_dataset test.txt   --model_epoch 38   --num_gen 100
