...
class IFNModule(Dataset):
    def __init__(self, data_dir, transform=None):
        ...
        self.token_paths = [p.replace('.png', '.token') for p in self.image_paths]
...
def parse_args():
    import argparse
    parser = argparse.ArgumentParser(description='ResNet50 IFN Backbone')
    ...
    p_train.add_argument('--from_scratch', action='store_true', help='Do not load ImageNet pretrained weights')
    p_train.add_argument('--device', type=str, default='cuda' if torch.cuda.is_available() else 'cpu')

    # reference settings
    p_ref = sub.add_parser('reference')
    p_ref.add_argument('--image_path', type=str, required=True)
    p_ref.add_argument('--ckpt', type=str, required=True, help='Path to trained .pth checkpoint')
